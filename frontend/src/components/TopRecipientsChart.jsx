...
export default function TopRecipientsChart({ data }) {
  ...
  if (!data || data.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow border border-gray-100 dark:border-gray-800 p-6">
        <h2 className="text-xl font-bold text-gray-900 dark:text-white mb-4">Top Recipients</h2>
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 p-4 sm:p-6 analytics-chart-container">
      <h2 className="text-lg sm:text-xl font-bold text-gray-900 dark:text-white mb-6">Top Recipients</h2>
      
      {selectedPoint && (
        <div className="mb-4 p-3 bg-purple-50 dark:bg-purple-950/30 rounded-xl border border-purple-200 dark:border-purple-900/50 animate-fade-in">
          ...
          <p className="text-sm text-gray-700 dark:text-gray-300">
            ...
            {' | '}
            Tips: <span className="font-bold text-purple-600 dark:text-purple-400">{selectedPoint.count}</span>
          </p>
        </div>
      )}
              ...
              >
                ...
                <div className="flex-1 bg-gray-50 dark:bg-gray-800 rounded-full h-7 overflow-hidden border border-gray-100 dark:border-gray-850 relative">
                  <div
                    ...
                    className={`h-full bg-gradient-to-r from-purple-500 to-indigo-600 rounded-full transition-all duration-500 relative flex items-center justify-end px-3 ${
          ...
          {data.slice(0, 5).map((recipient, index) => (
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <span className="text-xs sm:text-sm font-mono text-gray-700 dark:text-gray-300">{formatAddress(recipient.address)}</span>
              </div>
              <div className="text-right">
                <p className="text-xs sm:text-sm font-bold text-gray-900 dark:text-white">{formatAmount(recipient.volume)} STX</p>
                <p className="text-xs text-gray-455 dark:text-gray-500">{recipient.count} tips</p>
              </div>
            </div>
          ))}
        </div>
